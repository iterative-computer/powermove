/* Powermove — isolated JavaScript tools.
   ...
   command-building SDK. The only data accepted back is a bounded list of typed
   PM.Edit commands, which is validated again before preview or atomic apply. */
(() => {
const PM = window.PM;
...
function validateLocks(commands, state) {
  const locked = new Set((state.layers || []).filter(layer => layer.locked).map(layer => layer.id));
  for (const command of commands) {
    ...
    if (targets.some(target => locked.has(target))) throw new Error('Scripts cannot change locked layers');
  }
}

function normalizeCommands(raw, state) {
  ...
  const commands = source.map(command => PM.AgentHarness?.cleanCommand?.(command));
  if (commands.some(command => !command)) throw new Error('The script returned an unsupported or malformed source edit');
  if (commands.some(command => DENIED_OUTPUTS.has(command.type)
...
function runIsolated(code, project, input = {}) {
  return new Promise((resolve, reject) => {
    if (!document?.body) { reject(new Error('The isolated runtime needs a browser document')); return; }
    ...
    const timer = setTimeout(() => finish(new Error('The isolated runtime did not respond')), LIMITS.runtime + 900);
...
function describe(commands) {
  return commands.map(command => PM.AgentHarness?.describeCommand?.(command) || command.type);
}

async function prepare(code, input = {}, meta = {}) {
  ...
  try {
    ...
    return {
      ...
      changes: describe(commands).map(description => ({ description })),
...
async function preview(code, input, meta) { return prepare(code, input, meta); }

async function apply(code, input = {}, meta = {}) {
  ...
  if (applied.ok) {
    const added = prepared.commands.filter(command => command.type === 'add_layer' && command.id).map(command => command.id);
...
function canRun(action) {
  ...
  return selected.some(layer => required.has(layer.type) && !layer.lock);
}
