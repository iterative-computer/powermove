...
test('compact layer rows keep names centered and parenting in dedicated controls',async({session})=>{
  ...
  await page.evaluate(()=>{
    ...
    PM.replaceProject(p);PM.theme.apply('dark');PM.selectLayers([p.layers[3].id,p.layers[7].id]);PM.TL.scrollY=0;PM.invalidate();
  });
  await page.waitForFunction(()=>(window as any).PM.TL.rows.length===9);
  const rows=()=>page.evaluate(()=>{const T=(window as any).PM.TL,r=T.cv.getBoundingClientRect();return {x:r.left,y:r.top,gut:T.gut,row:T.row,ruler:T.ruler,height:r.height};});
  ...
  await page.waitForFunction(()=>(window as any).PM.TL.rows.some((r:any)=>r.L.name==='Nested'));
  await page.mouse.click(box.x+76,box.y+box.ruler+box.row*9.5);
  await page.waitForFunction(()=>(window as any).PM.TL.rows.some((r:any)=>r.L.name==='Prompt'));
