...
async function clean(page:any){await page.waitForFunction(()=>(window as any).PM?.Viewer?.ov && (window as any).PM?.GL?.gl);await page.evaluate(()=>{const PM=(window as any).PM;PM.replaceProject(PM.mkProject({name:'AE fundamentals QA',w:640,h:360,fps:30,dur:2,bg:'#000000'}));PM.setTime(0,{force:true});PM.selectLayers([]);PM.Viewer.fit=true;PM.Viewer.layout();});}
async function point(page:any,x:number,y:number){const b=await page.locator('#stage-inner').boundingBox(),z=await page.evaluate(()=>(window as any).PM.Viewer.shown);return {x:b.x+x*z,y:b.y+y*z};}
...
test('editable Pen, canvas text, graph and preview controls render coherently',async({session})=>{
 ...
 await expect(page.getByRole('button',{name:'Edit vertices'})).toBeVisible();
 ...
 await page.getByRole('combobox',{name:'Preview resolution',exact:true}).selectOption('0.25');expect(await page.evaluate(()=>[(window as any).PM.GL.canvas.width,(window as any).PM.GL.canvas.height])).toEqual([160,90]);
...
test('multiple graph curves, velocity editing and a cached preview retain editable animation',async({session})=>{
 ...
 const graphKey=await page.evaluate(()=>{const T=(window as any).PM.TL,p=T._graph.points.find((p:any)=>p.x>T.gut+8);return {x:p.x,y:p.y};});
...
test('a drawn mask, luma matte and live text selector change pixels and undo',async({session})=>{
 ...
 await page.getByRole('button',{name:/Pen Tool/}).click();for(const [x,y] of [[100,100],[300,100],[300,240],[100,240],[100,100]]){const p=await point(page,x!,y!);await page.mouse.click(p.x,p.y);}
