...
(() => {
  ...
  if (!window.powermove) return;

  const PM = window.PM;
  const bridge = window.powermove;
  ...
  function savedName(path, fallback) {
      ...
      pmCodex: {
        postMessage(body = {}) {
          ...
          void (async () => {
            ...
            const result = await bridge.codex.run(request, text => {
      ...
      pmAgentReveal: {
        postMessage(body = {}) {
          void bridge.artifacts.reveal({
            projectId: String(body.projectId || ''),
            path: String(body.path || '')
          }).catch(error => PM.toast(errorText(error, 'The artifact could not be revealed')));
        }
      },
      pmCaptureWindow: {
        postMessage(body = {}) {
          const id = String(body.id || '');
          void bridge.captureWindow().then(data => {
  ...
  const serviceKey = key => String(key).startsWith('pm.') ? String(key).slice(3) : String(key);
  const legacyKey = key => `pm.${serviceKey(key)}`;
  ...
  async function finishBoot() {
    ...
    if (!await publishSystemFonts() && typeof queryLocalFonts === 'function') {
      addEventListener('pointerdown', () => { void publishSystemFonts(); }, {
        capture: true,
        once: true
      });
    }

    bridge.onMenuCommand(command => PM.cmd && PM.cmd(command));
    setTimeout(() => {
      try {
