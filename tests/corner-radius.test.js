...
test('toolbar is inline with the native titlebar instead of drawing a second shell', () => {
  ...
  assert.match(css, /#toolbar-strip\s*\{[^}]*height:32px[^}]*background:transparent[^}]*border:0[^}]*border-radius:0/s);
...
test('all representative app chrome shares 60 percent corner smoothing without touching composition output', () => {
  ...
  assert.match(css, /:where\(button,input,textarea,select,\.panel,\.drop,\.modal,#library-screen,\.library-card,\.library-thumb,\.spatial-compose,\.spatial-hint,\.spatial-outline,\.panel-ghost-card,\.panel-drop-preview,\.pop-bar,\.pop-mirror\)/);
