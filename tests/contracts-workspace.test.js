...
test('normalization adds one flex dock only when none exists and preserves multiple authored flex docks', () => {
...
test('retired and duplicate panel ids are removed from visible docks', () => {
  const WS = workspaceModel();
...
test('workspace numeric dimensions are clamped to their contract bounds', () => {
  const WS = workspaceModel();
...
test('custom tool state keeps at most 64 primitive-valued keys', () => {
  const WS = workspaceModel();
...
test('controls without a valid type infer select, toggle, color, text, and slider', () => {
  const WS = workspaceModel();
  const workspace = WS.normalize(manifest({
