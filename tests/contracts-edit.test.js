...
test('Edit.operations freezes the complete source-edit vocabulary', () => {
  const { PM } = fixture();
...
test('all 18 source operations apply successfully and have an observable effect', () => {
...
test('scene parameter edits preserve the existing parameter object identity', () => {
  const { PM } = fixture();
...
test('baseRevision mismatch rejects the edit with the project-changed contract', () => {
...
test('a five-dispatch live edit coalesces to one edit and one revision', () => {
  const { PM, text } = fixture();
...
test('cancelling an empty live edit preserves the project object reference', () => {
  const { PM } = fixture();
...
test('the source-edit provenance log is capped at 200 entries', () => {
  const { PM, text } = fixture();
