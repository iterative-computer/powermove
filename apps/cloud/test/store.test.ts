...
import { Store, type ListingDto } from '@powermove/registry/wire';
...
const post = (body: unknown, headers: Record<string,string> = {}) => ({ method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
...
test('browse returns a clean internal error without a database binding', async () => {
  const response = await createApp({ data: env => neonData(env.DATABASE_URL) }).request('/v1/store/browse', {}, { APP_ORIGIN: 'https://cloud.trypowermove.com' } as CloudflareBindings);
  expect(response.status).toBe(500);
  expect(await json(response)).toEqual({ error: 'internal' });
});

test('browse, search, category, cursor, sort and fork lineage', () => withData(async data => {
  ...
  const first = await json(await publishRequest(data, env, alice, 'demo', built.commitSha, { listing: { name: 'Aurora Glow', tagline: 'Color bloom', category: 'effects' } }));
  const secondBuilt = await uploadTree(data, env, alice, files('spark'));
  const second = await json(await publishRequest(data, env, alice, 'spark', secondBuilt.commitSha, { listing: { name: 'Spark Trail', tagline: 'Bright glow', category: 'effects' } }));
  const forkBuilt = await uploadTree(data, env, bob, [
    { path: 'manifest.json', bytes: enc.encode(JSON.stringify({ id: 'fork', name: 'Fork', version: '1.0.0', apiVersion: 1, forkedFrom: 'alice/demo@1.0.0' })) },
    { path: 'index.ts', bytes: enc.encode('export default 2') }
  ], { parents: [built.commitSha] });
  const fork = await json(await publishRequest(data, env, bob, 'fork', forkBuilt.commitSha, { originReleaseId: first.release.id, listing: { name: 'Aurora Fork', tagline: 'A remix', category: 'tools' } }));
  await data.db.insert(featured).values([{ repoId: second.repo.repoId, section: 'featured', position: 1 }, { repoId: first.repo.repoId, section: 'featured', position: 0 }, { repoId: fork.repo.repoId, section: 'picks', position: 0 }]);
  ...
  for (const literal of ['%25', '_', '%5C']) expect((await json(await app.request(`/v1/store/extensions?q=${literal}`, {}, env))).items).toHaveLength(0);
  ...
  for (const sort of ['new', 'installs', 'name']) {
    ...
    do {
      const page: any = await json(await app.request(`/v1/store/extensions?sort=${sort}&limit=1${cursor ? `&cursor=${cursor}` : ''}`, {}, env));
      seen.push(...page.items.map((x: ListingDto) => x.repoId)); cursor = page.nextCursor;
    } while (cursor);
...
test('every store read follows lifecycle and yanked releases fall back', () => withData(async data => {
  const env = makeEnv(data), alice = await publisher(data, env, 'alice'), bob = await publisher(data, env, 'bob');
  const built = await uploadTree(data, env, alice, files('demo'));
  const first = await json(await publishRequest(data, env, alice, 'demo', built.commitSha));
  const nextBuilt = await uploadTree(data, env, alice, files('demo', '2.0.0', [{ path: 'extra.txt', bytes: enc.encode('new') }]), { parents: [built.commitSha] });
  ...
  const changed = await json(await app.request(`/v1/store/compare?base=${first.release.id}&head=${releaseId}`, {}, env));
  ...
  const paths = [
    '/v1/store/x/alice/demo/r/2.0.0', '/v1/store/x/alice/demo/r/2.0.0/tree', '/v1/store/x/alice/demo/r/2.0.0/tar',
    '/v1/store/x/alice/demo/r/2.0.0/file/index.ts', `/v1/store/releases/${releaseId}`, `/v1/store/releases/${releaseId}/tree`, `/v1/store/releases/${releaseId}/tar`,
    `/v1/store/compare?base=${first.release.id}&head=${releaseId}`
  ];
  const check = async (expected: { browse: boolean; detail: number; content: number; state: string; latest: string | null }, headers: Record<string,string> = {}) => {
    const browse = await json(await app.request('/v1/store/browse', { headers }, env));
    expect(browse.sections.find((s: any) => s.id === 'new').items.some((i: any) => i.repoId === repoId)).toBe(expected.browse);
    expect((await app.request('/v1/store/x/alice/demo', { headers }, env)).status).toBe(expected.detail);
    for (const path of paths) expect((await app.request(path, { headers }, env)).status).toBe(expected.content);
    const versions = await json(await app.request('/v1/store/versions', post({ items: [{ repoId, releaseId }] }, headers), env));
  ...
  for (const path of paths) expect((await app.request(path, {}, env)).status).toBe(410);
  const versions = await json(await app.request('/v1/store/versions', post({ items: [{ repoId, releaseId }, { repoId: crypto.randomUUID(), releaseId }] }), env));
  expect(versions.items[0]).toMatchObject({ state: 'ok', current: { yanked: true }, latest: { releaseId: first.release.id } });
  expect(versions.items[1]).toMatchObject({ state: 'removed', ownerPublisherId: null, latest: null, handle: null, slug: null });
  ...
  const onlyYanked = await json(await app.request('/v1/store/versions', post({ items: [{ repoId, releaseId }] }), env));
  expect(onlyYanked.items[0].latest).toBeNull();
}));

test('file headers, comparison, path rejection and install count lifecycle', () => withData(async data => {
  ...
  expect(tar.headers.get('Cache-Control')).toBe('public, max-age=31536000, immutable');
  ...
  expect((await app.request(`/v1/installs/${repoId}`, { method: 'DELETE', headers: alice.headers }, env)).status).toBe(204);
  ...
  const settings = await app.request('/v1/me/settings', { method: 'PATCH', headers: { ...alice.headers, 'Content-Type': 'application/json' }, body: JSON.stringify({ rememberInstalls: false }) }, env);
