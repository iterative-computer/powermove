...
import { canBrowse,canViewDetail,canReadFiles,updateOffered,type RepoState } from '../src/lifecycle';
...
] as const)('%s lifecycle',(_name,patch,browse,detail,read,offered)=>{
  const repo={...base,...patch} as RepoState,release={yankedAt:_name==='yanked'?new Date():null};
  expect(canBrowse(repo,other)).toBe(browse);expect(canViewDetail(repo,other)).toBe(detail);expect(canReadFiles(repo,release,other)).toBe(read);expect(updateOffered(repo,release)).toBe(offered);
  if(_name==='hidden'){expect(canViewDetail(repo,owner)).toBe('ok');expect(canReadFiles(repo,release,owner)).toBe('ok');}
});
