...
import { type BuiltinPlan, publishBuiltins } from '../../desktop/scripts/lib/publish-builtins';
...
test('publishes ten built-ins, skips unchanged versions, and requires a bump for changed trees', () =>
  withData(async (data) => {
    ...
    expect(Object.fromEntries(first.map((plan) => [plan.coordinate.split('/')[1], plan.category]))).toEqual(
      expectedCategories,
    );
    ...
    await expect(publishBuiltins({
      ...options,
      dirs: [resolve(root, 'timeline')],
      readDir: async (dir) => {
