...
test('editable Pen, canvas text, graph and preview controls render coherently',async({session})=>{
 ...
 await page.locator('#preview-controls select').selectOption('0.25');expect(await page.evaluate(()=>[(window as any).PM.GL.canvas.width,(window as any).PM.GL.canvas.height])).toEqual([160,90]);
