...
test.describe('@groups editable timeline groups', () => {
  ...
  test('grouping preserves rendered pixels, time, parent rigs, save data and Undo', async ({ session }) => {
    const { page } = session;
    await page.waitForFunction(() => { const PM = (window as any).PM, timeline = PM.Kernel.services.get('timeline'); return Boolean(PM?.GL?.gl && timeline?.rows); });
    ...
    const twirl = await page.evaluate(() => { const PM=(window as any).PM,timeline=PM.Kernel.services.get('timeline'),r=timeline.cv.getBoundingClientRect(); return {x:r.left+64,y:r.top+timeline.ruler+timeline.row/2-timeline.scrollY}; });
    await page.mouse.click(twirl.x,twirl.y);
    await page.waitForFunction(() => { const PM=(window as any).PM,timeline=PM.Kernel.services.get('timeline'); return timeline.rows.length === 1; });
    await page.mouse.click(twirl.x,twirl.y);
    await page.waitForFunction(() => { const PM=(window as any).PM,timeline=PM.Kernel.services.get('timeline'); return timeline.rows.length === 3; });
    await page.screenshot({path:'/tmp/powermove-groups-review.png'});
    const clip = await page.evaluate(() => { const PM=(window as any).PM,timeline=PM.Kernel.services.get('timeline'),r=timeline.cv.getBoundingClientRect();return{x:r.left+timeline.gut+(1.5-timeline.scrollT)*timeline.pps,y:r.top+timeline.ruler+timeline.row/2-timeline.scrollY,delta:timeline.pps/2}; });
  ...
  test('canvas and timeline open the same compact layer menu', async ({ session }) => {
    const {page}=session;
    await page.waitForFunction(()=>{const PM=(window as any).PM,timeline=PM.Kernel.services.get('timeline');return Boolean(PM?.GL?.gl&&timeline?.cv);});
    await page.evaluate(()=>{
      const PM=(window as any).PM;
    ...
    const canvasItems = await chooseNativeMenu(session, 'Lock', () => page.mouse.click(coords.x,coords.y,{button:'right'}));
    ...
    await chooseNativeMenu(session, 'Unlock', () => page.mouse.click(coords.x,coords.y,{button:'right'}));
    ...
    const timelineItems = await chooseNativeMenu(session, 'Group layers', () => page.mouse.click(row.x,row.y,{button:'right'}));
    expect(timelineItems).toEqual(canvasItems);
  ...
  test('pickwhip parents to a timeline row with no pose jump and one Undo',async({session})=>{
    ...
    await page.waitForFunction((id)=>{const PM=(window as any).PM,timeline=PM.Kernel.services.get('timeline');return timeline.rows.some((row:any)=>row.kind==='layer'&&row.L.id===id);},ids.parent);
    const end=await page.evaluate((id)=>{const PM=(window as any).PM,timeline=PM.Kernel.services.get('timeline'),r=timeline.cv.getBoundingClientRect(),index=timeline.rows.findIndex((row:any)=>row.kind==='layer'&&row.L.id===id);return{x:r.left+110,y:r.top+timeline.ruler+index*timeline.row+timeline.row/2-timeline.scrollY};},ids.parent);
