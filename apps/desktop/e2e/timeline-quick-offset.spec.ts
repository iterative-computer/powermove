...
async function waitForTimeline(page: any) {
  await page.waitForFunction(() => Boolean((window as any).PM?.TL?.cv && (window as any).PM?.Edit));
}
...
test.describe('@timeline Quick Offset', () => {
  test('stagger layers in selection order with a subframe Cmd+Option drag and one undo', async ({ session }) => {
    ...
    const overlay = await page.evaluate(() => {
      ...
      const row = PM.TL.rows.findIndex((item: any) => item.kind === 'layer' && item.L.id === layers[1].id);
      const rect = PM.TL.cv.getBoundingClientRect();
      const x = rect.left + PM.TL.gut + 150;
      const y = rect.top + PM.TL.ruler + row * PM.TL.row + PM.TL.row / 2;
      ...
      PM.TL.cv.dispatchEvent(event('pointerdown', x));
      window.dispatchEvent(event('pointermove', x + 125));
      const overlay = { ...PM.TL.quickOffset };
    ...
    expect(await page.evaluate(() => (window as any).PM.TL.quickOffset)).toBeNull();
  ...
  test('stagger selected keyframe groups per layer without changing each group span', async ({ session }) => {
    ...
    const setup = await page.evaluate(() => {
      ...
      PM.replaceProject(project);
      layers.forEach(layer => PM.TL.reveal(layer, ['opacity']));
      PM.sel.layers = [layers[1].id, layers[0].id, layers[2].id];
      PM.sel.keys = layers.flatMap(layer => layer.p.opacity.kf.map((key: any) => key.i));
      PM.sel.chan = 'opacity'; PM.TL.keySelectionActive = true;
      PM.TL.pps = 100; PM.TL.scrollT = 0;
    ...
    await page.evaluate(() => {
      ...
      const row = PM.TL.rows.findIndex((item: any) => item.kind === 'prop' && item.L.id === layers[1].id && item.key === 'opacity');
      const rect = PM.TL.cv.getBoundingClientRect();
      const x = rect.left + PM.TL.gut + 100;
      const y = rect.top + PM.TL.ruler + row * PM.TL.row + PM.TL.row / 2;
      const event = (type: string, clientX: number) => new PointerEvent(type, {
        bubbles: true, cancelable: true, button: 0, buttons: type === 'pointerup' ? 0 : 1,
...
test('one Graph Editor handle adjusts selected same-property keyframes across layers', async ({ session }) => {
  ...
  await page.evaluate((id: string) => {
    const PM = (window as any).PM, rect = PM.TL.cv.getBoundingClientRect();
    ...
    PM.TL.cv.dispatchEvent(event('pointerdown', x, y));
