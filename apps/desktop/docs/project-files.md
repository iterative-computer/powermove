...
Each replacement keeps the previous complete version in the app's Application Support `backups/<project id>/` folder, with the newest 5 versions retained. You can open a backup with Command+O, then use Save As to give the recovered version its own `.pmv` name. Writes use a flushed temporary file followed by an atomic rename. If a backup or write fails, Powermove reports the error and does not report the project saved. If the file has been moved, removed, or changed by another app or open copy, use Save As; Powermove will not silently overwrite those changes.
