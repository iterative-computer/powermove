...
The complete local checkout was captured as 1,087 tracked and untracked source files and reconciled with main at `f4f4c4632dd147b00681c447c6e0fedf5cc32174`. Of 175 locally changed files, 66 were already present on main; the remaining 109 are included in the integration. Merge resolution preserves current remote-host support alongside local history compaction, synchronized layered-video playback, transparent seek-frame replacement, preview reuse, and media-restoration state.

Fresh validation of the combined tree passes 2,441 standard unit tests plus the separately enabled 5,000-layer performance-program test (2,442 total). Six standalone production-history tests and three website release-metadata tests pass. Desktop, player, website, and CLI production builds pass; desktop and website type checks report zero errors and warnings; all ten extension boundary checks pass.

The historical memory measurements and performance limitations above remain scoped to the September 19 workloads; they are not new whole-app measurements of this combined tree.

The full desktop UI run contained 354 cases: 339 passed, two failed, and 13 optional cases skipped. Both failures were fixture races: an attachment conversation injected only into view state was replaced by a later state publication, and a second timeline-disclosure click reused coordinates before the canvas/layout frame completed. The attachment fixture now loads a saved thread; the disclosure fixture waits for presentation and recomputes coordinates. All three cases in those two files passed five repetitions each (15/15), clearing both failures. Thus all **341 available functional UI cases passed across the full run and focused reruns**, including document history across relaunch, synchronized layered video, transparent scrubbing, native export, and the project round trip over 1 GiB. Optional private/live/performance fixtures remain explicitly unexecuted in this run.
