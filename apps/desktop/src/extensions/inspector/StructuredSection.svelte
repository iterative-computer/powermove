<script lang="ts">
  ...
  import { makeVectorPath, pathTargets, structuredProperties } from 'powermove';
  ...
  const activeValues=$derived.by(()=>{
    ...
    return Object.fromEntries(Object.entries(activeTarget.path.p||{}).map(([key,property])=>[
      key,PM.evP(layer,property,transport.time,`${activeTarget.prefix}.${key}`)
    ]));
  });
  ...
  function applyPathValue(key:string,value:any,label:string){
    ...
    PM.Edit.apply({type:'set_property',target:layer.id,path:channel(key),value,time:transport.time,preserveHandEdits:false},{label,origin:'inspector'});
    PM.invalidate();
  }
  function selectPath(target:any,edit=false){activePathId=target.path.id;PM.activePath=target.path.id;if(edit)PM.setTool('pen');}
  function addPath(){mutate('Add path',()=>{const path=makeVectorPath(PM);(layer.d.paths||=[]).push(path);activePathId=path.id;PM.activePath=path.id;});PM.setTool('pen');}
  function removePath(target:any){mutate('Remove path',()=>{layer.d.paths=layer.d.paths.filter((path:any)=>path.id!==target.path.id);for(const path of layer.d.paths)if(path.parent===target.path.id)path.parent=null;activePathId=layer.d.paths[0]?.id??null;PM.activePath=activePathId;});}
...
{#if layer.type==='shape' && activeTarget}
  ...
  <details class="advanced">
    ...
    {#if shapeTargets.length>1}
      <Row label="Group"><SelectField {PM} label="Path parent group" get={()=>activeTarget.path.parent} options={[{v:null,label:'None'},...shapeTargets.filter((target:any)=>target!==activeTarget&&!target.path.parent).map((target:any)=>({v:target.path.id,label:target.path.name}))]} edit={{mode:'set',label:'Group path',set:(value:any)=>{activeTarget.path.parent=value;PM.invalidate();inspectorRefresh.bump();}}} /></Row>
    {/if}
    <div class="advanced-label">Path transform</div>
    {#each ['x','y','rotation','scaleX','scaleY'] as key (key)}
      <ChannelRow {PM} {layer} channel={channel(key)} property={property(key)} label={label(key)} step={1} unit={key==='rotation'?'°':key.startsWith('scale')?'%':undefined} />
    {/each}
    <AnimatedRow {PM} {layer} path={channel('closed')} label="Closed">
    ...
    <ChannelRow {PM} {layer} channel={channel('trimStart')} property={property('trimStart')} label="Start" step={1} min={0} max={100} unit="%" />
    <ChannelRow {PM} {layer} channel={channel('trimEnd')} property={property('trimEnd')} label="End" step={1} min={0} max={100} unit="%" />
    <ChannelRow {PM} {layer} channel={channel('trimOffset')} property={property('trimOffset')} label="Offset" step={1} unit="°" />
    <div class="advanced-label">Repeater</div>
    <ChannelRow {PM} {layer} channel={channel('copies')} property={property('copies')} label="Copies" step={1} min={1} max={256} />
    {#if Number(activeValues.copies)>1}
      <ChannelRow {PM} {layer} channel={channel('repeatX')} property={property('repeatX')} label="Offset X" step={1} />
      <ChannelRow {PM} {layer} channel={channel('repeatY')} property={property('repeatY')} label="Offset Y" step={1} />
      <ChannelRow {PM} {layer} channel={channel('repeatRotation')} property={property('repeatRotation')} label="Rotation" step={1} unit="°" />
    {/if}
  </details>
  ...
  {#if activeValues.fillEnabled}
    <AnimatedRow {PM} {layer} path={channel('fill')} label="Color"><ColorField {PM} label="Fill color" get={()=>activeValues.fill} edit={bind(channel('fill'))} /></AnimatedRow>
    <ChannelRow {PM} {layer} channel={channel('fillOpacity')} property={property('fillOpacity')} label="Opacity" step={1} min={0} max={100} unit="%" />
  {/if}
  ...
  {#if Number(activeValues.strokeWidth)>0}
    <AnimatedRow {PM} {layer} path={channel('stroke')} label="Color"><ColorField {PM} label="Stroke color" get={()=>activeValues.stroke} edit={bind(channel('stroke'))} /></AnimatedRow>
    <ChannelRow {PM} {layer} channel={channel('strokeWidth')} property={property('strokeWidth')} label="Weight" step={0.5} min={0} unit="px" />
    <ChannelRow {PM} {layer} channel={channel('strokeOpacity')} property={property('strokeOpacity')} label="Opacity" step={1} min={0} max={100} unit="%" />
  {/if}
{/if}
  ...
  {#each maskTargets as target (target.path.id)}
    <details><summary>{target.path.name}</summary>
      ...
      {#each records.filter((record:any)=>record.key.startsWith(target.prefix+'.')&&!record.key.includes('.v.')) as record (record.key)}
        ...
        {:else}<AnimatedRow {PM} {layer} path={record.key} label={label(record.label)}>{#if typeof value==='boolean'}<button class="compact-toggle" class:on={!!value} onclick={()=>PM.Edit.apply({type:'set_property',target:layer.id,path:record.key,value:!value,time:transport.time,preserveHandEdits:false},{label:'Edit '+record.key,origin:'inspector'})}>{value?'On':'Off'}</button>{:else}<ColorField {PM} label={label(record.label)} get={()=>value} edit={bind(record.key)} />{/if}</AnimatedRow>{/if}
      {/each}
    </details>
  {/each}
{/if}
