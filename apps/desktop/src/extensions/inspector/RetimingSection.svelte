...
{#if layer.d.sourceTime}
  <AnimatedRow {layer} path="c.timeRemap" label="Time remap">
    <ToggleField {...controlProps} get={()=>!!enabled} label="Time remap" edit={{mode:'command',label:'Time remap',origin:'inspector',command:(value:any)=>({type:'set_property',target:layer.id,path:'c.timeRemap',value:!!value,preserveHandEdits:false})}} />
  </AnimatedRow>
