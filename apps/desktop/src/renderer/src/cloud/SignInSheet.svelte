...
import googleMark from './google.svg';
        ...
        <div class="acct-providers">
          {#each PROVIDERS as p (p)}
            <button class="btn acct-provider" type="button" data-provider={p} onclick={() => void viaProvider(p)}>
              {#if p === 'google'}
                <!-- Google's brand mark from svgl (svgl.app/library/google.svg). -->
                <img class="acct-provider-mark" src={googleMark} alt="" width="16" height="16" draggable="false" />
              {:else}
                <Icon {PM} name={p} />
              {/if}
