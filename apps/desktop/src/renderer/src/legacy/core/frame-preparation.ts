...
export async function prepareFrame(PM:any,time:number,project=PM.proj):Promise<void> {
  const requests:any[]=[];
  const visit=(comp:any,t:number,depth:number)=>{if(depth>8)throw new Error('Composition nesting is too deep');PM.scope.push(comp);try{for(const layer of comp.layers){if(!PM.active(layer,t))continue;if(layer.type==='video'){const asset=PM.assets.get(layer.d.asset);if(asset?.el)requests.push({layer,asset,time:t,at:Math.max(0,Math.min(Math.max(0,asset.dur-1/PM.proj.fps),sourceTime(PM,layer,t)))});}else if(layer.type==='precomp'){const sub=comp.comps?.[layer.d.comp]||PM.proj.comps?.[layer.d.comp];if(sub)visit(sub,sourceTime(PM,layer,t),depth+1);}}}finally{PM.scope.pop();}};
  visit(project,time,0);const frames=new Map();
  for(const r of requests){await seek(r.asset.el,r.at);const cv=document.createElement('canvas');cv.width=r.asset.w||r.asset.el.videoWidth;cv.height=r.asset.h||r.asset.el.videoHeight;cv.getContext('2d')!.drawImage(r.asset.el,0,0,cv.width,cv.height);frames.set(r.layer.id+'@'+r.time,cv);}
