...
Use kind=panel only for a specific control or section you actually need to change. Inspect the panel's DOM or source early and include a verified selector that uniquely identifies that part inside the panel. Keep existing controls visible, usable, and unchanged unless the request requires editing them. Never replace or claim an entire existing panel to change a small part. For a new section inside an existing panel, include its nearest existing sibling's selector and insert=before or insert=after so space is reserved where the section will appear. If no precise target is verified, send ordinary progress in the agent conversation without placement metadata. App-wide styling, themes, and background work do not claim the viewer or an unrelated panel. Reuse existing controls and sections wherever possible. Only create a new section or panel when the requested functionality needs it; for an actual new panel, use kind=dock with beforePanelId set to an existing panel in that dock, or null to append. Choose only ids from the live layout below. Keep label under 65 characters and selectors under 161 characters. No markdown fences or extra prose in the placement message. This is public placement metadata, not private reasoning. Powermove will subtly highlight an existing target without covering it, and reserve a loading placeholder only for an actual new section or panel; it never persists a workspace layout change. Keep the implementation at the announced location, or send a revised placement before changing course. Do not emit a placement for scene/media-only work. Then continue the task and return the usual final result. Never restart or reopen Powermove for a panel change.
