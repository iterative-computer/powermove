...
Unless the user explicitly requests a different visual style, every new or changed panel must match Powermove's existing design language one to one, as if it shipped with the app. Use api.panels.register and let the host supply the normal panel frame, header, docking and scrolling; do not draw a second card or title bar. Reuse api.ui.controls and the nearest built-in panel's patterns for fields, rows, sections, buttons and icons. Match its spacing, control heights, label alignment, typography, corner radii, borders and surfaces using the app's CSS tokens (--f-ui, --fs-md, --row-h, --ctl-h, --r-base, --bg-panel, --bg-field, --tx, --tx-2, --line, --accent). Inherit light/dark themes; do not hardcode a separate palette, oversized headings, gradients, decorative cards or a custom design system. Use flat tonal fills or transparent buttons; never add bevels, top highlights, or raised button shadows. Group error recovery actions inside their borderless error surface. Keep keyboard focus and meaningful selection indicators visible. Preserve normal panel chrome when modifying an existing panel. An explicit user style request overrides this default only for the requested surface. Keep controls connected to editable source and normal Undo.
PANEL ICONS: Every new panel must declare a meaningful icon in api.panels.register({ id, title, icon, ... }) (or section.icon for a generated section). Choose it yourself based on the panel's purpose; do not use the generic panel icon or repeat an existing panel's icon. Valid names include frame, wand, project, puzzle, note, speedometer, image, sparkle, sliders, code, layers, timeline, tools, grid, music, film, graph, clock, cam, type, shape, diamond, gear, list, link, sun, and eye. Inspect the current panel icons and pick an unused suitable glyph; keep the same icon on later edits.`;
