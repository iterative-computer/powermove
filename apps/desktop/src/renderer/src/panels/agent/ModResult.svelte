...
<div class="agent-mod-result" class:needs-attention={result.status === 'error'}>
  <div class="mod-mark" aria-hidden="true">
    {#if result.action === 'updated'}
      <svg viewBox="0 0 24 24"><path d="M14 4H5v16h14v-9 M11 13l1-4 7-7 3 3-7 7-4 1Z" /></svg>
    {:else}
      <svg viewBox="0 0 24 24"><path d="m12 3 9 5-9 5-9-5Z M3 12l9 5 9-5 M3 16l9 5 9-5" /></svg>
    {/if}
  </div>
  <div class="mod-copy">
    <small>{result.action === 'updated' ? (panels.length > 1 ? `${panels.length} panels edited` : 'Panel edited') : 'Panel created'}</small>
    <b>{result.name}</b>
  </div>
  {#if panels.length === 1}
