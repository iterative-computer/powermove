<script lang="ts">
  ...
  function setHex(next: unknown): boolean {
  ...
  function apply(): void {
    if (!setHex(draft)) { PM.toast?.('Enter a six-digit hex color'); return; }
    gesture.once(chosen);
...
{#if open}
  <div class="fill-picker-layer" role="presentation" onpointerdown={(event) => { if (event.target === event.currentTarget) close(); }}>
    <div bind:this={dialog} class="fill-picker color-picker" role="dialog" aria-modal="true" aria-label={label} tabindex="-1" style:left={`${pickerLeft}px`} style:top={`${pickerTop}px`} onkeydown={keydown}>
      ...
      <footer><button type="button" class="btn" onclick={close}>Cancel</button><button type="button" class="btn pri" onclick={apply}>OK</button></footer>
    </div>
  </div>
{/if}
