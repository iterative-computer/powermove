...
export function parseUIPlacement(text: string, workspace: Workspace | null | undefined): UIPlacement | null {
  ...
  try {
    ...
    if (value.kind === 'panel' && docks.some(dock => dock.panels.some(panel => panel.id === value.id))) {
      return { kind: 'panel', id: value.id, label };
    }
    if (value.kind === 'dock') {
...
Use kind=panel when modifying an existing visible panel. For a new panel, use kind=dock with beforePanelId set to an existing panel in that dock, or null to append. Choose only ids from the live layout below. Keep label under 65 characters. No markdown fences or extra prose in the placement message. This is public placement metadata, not private reasoning. Powermove will show a click-through loading ghost there while you work; it does not change the layout. Keep the implementation at the announced location, or send a revised placement before changing course. Do not emit a placement for scene/media-only work. Then continue the task and return the usual final result. Never restart or reopen Powermove for a panel change.
