...
describe('general settings control', () => {
  it('groups accounts and application preferences into labelled sections', () => {
    ...
    expect(accounts?.querySelectorAll('.settings-chatgpt')).toHaveLength(1);
    expect(control.element.textContent).toContain('Powermove never reads or stores your account token.');
    control.destroy();
  });
