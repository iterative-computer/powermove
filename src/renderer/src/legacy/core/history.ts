/* Ported from js/core/history.js — behavior-preserving. */
import type { PMRegistry } from '../registry';

export function install(PM: PMRegistry): void {
const MAX = 120;
let stack: any[] = [], idx = -1, pending: any = null, depth = 0;

const snap = () => JSON.stringify(PM.proj);
const restore = (json: any) => {
...
const H = {
  ...
  do(label: any, fn: any) { H.begin(label); try { return fn(); } finally { H.commit(label); } },
  undo() {
  ...
  squash(mark: any, label: any = 'Agent change', group: any = null) {
    ...
    const baseIndex = mark.topId ? stack.findIndex((entry: any) => entry.id === mark.topId) : -1;
    ...
    if (!entries.length || entries.some((entry: any) => entry.undo || entry.redo || group && entry.group !== group)) return null;
    const after = snap();
...
PM.takes = {
  ...
  restore(id: any) {
    const t = PM.takes.all().find((t: any) => t.id === id);
    if (!t) return false;
    PM.hist.do('Restore ' + t.label, () => restore(t.json));
    return true;
  },
  drop(id: any) { PM.store.set('takes', PM.takes.all().filter((t: any) => t.id !== id)); PM.bus.emit('takes'); },
};
}
