/* Ported from js/core/capabilities.js — behavior-preserving. */
import type { PMRegistry } from '../registry';

export function install(PM: PMRegistry): void {
const MAX_EDITS: any = 32;
const MAX_EXPR_DEPTH: any = 8;
const MAX_EXPR_NODES: any = 160;
...
const ORDERS: any = new Set(['stack', 'reverseStack', 'selection', 'reverseSelection', 'start', 'reverseStart', 'name', 'random']);
const OPS: any = new Set(['add', 'subtract', 'multiply', 'divide', 'min', 'max', 'clamp', 'round', 'floor', 'ceil', 'abs', 'negate', 'frames', 'equal', 'if']);
const OP_ARITY: any = Object.freeze({
...
const REFS: any = new Set(['current', 'index', 'count', 'fps', 'playhead', 'composition.duration']);
const AGGREGATES: any = new Set(['min', 'max', 'first', 'last', 'sum', 'average']);
const DEFAULT_CURVE: any = Object.freeze([.62, .05, 0, 1]);

const clone: any = (value: any) => value === undefined ? undefined : JSON.parse(JSON.stringify(value));
const cleanText: any = (value: any, fallback: any = '', max: any = 100) => typeof value === 'string' && value.trim()
  ? value.trim().slice(0, max) : fallback;
const finite: any = (value: any) => typeof value === 'number' && Number.isFinite(value);
const stateKey: any = (value: any) => typeof value === 'string' && SAFE_KEY.test(value) ? value : '';
const pathAllowed: any = (path: any) => LAYER_PATHS.has(path)
  || (/^properties\.[a-z0-9_.-]{1,120}$/i.test(path))
  || (/^content\.[a-z0-9_.-]{1,120}$/i.test(path));

function sanitizeCurve(value: any, fallback: any = null) {
  ...
  ].map((value: any) => PM.round(value, 3));
}

function sanitizeExpression(value: any, budget: any = { nodes: 0 }, depth: any = 0) {
  budget.nodes++;
  ...
  if (stateKey(value.state)) {
    const out: any = { state: value.state };
  ...
  if (OPS.has(value.op)) {
    const sourceArgs: any = (Array.isArray(value.args) ? value.args : []).slice(0, 8);
    const args: any = [];
    for (const sourceArg of sourceArgs) {
      const arg: any = sanitizeExpression(sourceArg, budget, depth + 1);
      if (arg === null && sourceArg !== null) return null;
      args.push(arg);
    }
    const [minArgs, maxArgs]: any = OP_ARITY[value.op];
...
function sanitizeOrder(value: any) {
...
function sanitizeTransform(raw: any) {
  let source: any = raw;
  if (typeof source === 'string') {
  ...
  const selectorSource: any = source.selector && typeof source.selector === 'object' ? source.selector : {};
  const selector: any = {
    ...
    types: (Array.isArray(selectorSource.types) ? selectorSource.types : [])
      .filter((type: any) => typeof type === 'string' && PM.TYPE_META?.[type]).slice(0, 16),
  };
  const edits: any = (Array.isArray(source.edits) ? source.edits : []).slice(0, MAX_EDITS).map((edit: any) => {
    const path: any = cleanText(edit?.path, '', 140);
    if (!pathAllowed(path)) return null;
    const expression: any = sanitizeExpression(edit?.value);
...
function sanitizeControlAction(raw: any) {
  let source: any = raw;
  if (typeof source === 'string') {
    if (source.length > 80_000) return null;
    try { source = JSON.parse(source); } catch { return null; }
  ...
  if (source.type === 'transform' && ['preview', 'apply'].includes(source.mode)) {
    const transform: any = sanitizeTransform(source.transform);
  ...
  if (source.type === 'easing' && ['preview', 'apply'].includes(source.mode)) {
    const curveState: any = stateKey(source.curveState);
  ...
  if (source.type === 'script' && ['preview', 'apply'].includes(source.mode)) {
    const code: any = typeof source.code === 'string' && source.code.trim().length <= 40_000 ? source.code.trim() : '';
    if (!code) return null;
    return {
...
function getPath(layer: any, path: any) {
  if (path.startsWith('layer.')) {
    const key: any = path.slice(6);
    const sourceKey: any = ({ duration: 'dur', visible: 'on', locked: 'lock', motionBlur: 'mblur' } as any)[key] || key;
    return layer[sourceKey];
  }
  if (path.startsWith('content.')) return layer.d?.[path.slice(8)];
  if (path.startsWith('properties.')) {
    const key: any = path.slice(11), prop: any = PM.findProp(layer, key);
    return prop ? PM.evP(layer, prop, PM.time, key) : undefined;
  }
  return undefined;
}

function resolveTargets(selector: any) {
  const projectLayers: any = PM.proj?.layers || [];
  let targets: any;
  if (selector.scope === 'selection') {
    const selected: any = new Set(PM.sel?.layers || []);
    targets = projectLayers.filter((layer: any) => selected.has(layer.id));
  } else if (selector.scope === 'visible') targets = projectLayers.filter((layer: any) => layer.on);
  else targets = [...projectLayers];
  if (selector.types.length) {
    const types: any = new Set(selector.types); targets = targets.filter((layer: any) => types.has(layer.type));
  }
  if (!selector.includeLocked) targets = targets.filter((layer: any) => !layer.lock);
  return targets;
}

function orderName(order: any, state: any) {
  if (typeof order === 'string') return order;
  const value: any = state?.[order.state];
...
function orderedTargets(targets: any, order: any, state: any, seed: any) {
  const next: any = [...targets], selectedOrder: any = new Map((PM.sel?.layers || []).map((id: any, index: any) => [id, index]));
  switch (orderName(order, state)) {
    case 'reverseStack': return next.reverse();
    case 'selection': return next.sort((a: any, b: any) => (selectedOrder.get(a.id) ?? 1e9) - (selectedOrder.get(b.id) ?? 1e9));
    case 'reverseSelection': return next.sort((a: any, b: any) => (selectedOrder.get(b.id) ?? -1) - (selectedOrder.get(a.id) ?? -1));
    case 'start': return next.sort((a: any, b: any) => a.from - b.from || PM.proj.layers.indexOf(a) - PM.proj.layers.indexOf(b));
    case 'reverseStart': return next.sort((a: any, b: any) => b.from - a.from || PM.proj.layers.indexOf(a) - PM.proj.layers.indexOf(b));
    case 'name': return next.sort((a: any, b: any) => a.name.localeCompare(b.name));
    case 'random': return next.sort((a: any, b: any) => stableHash(`${seed}:${a.id}`) - stableHash(`${seed}:${b.id}`));
    default: return next;
  }
}

function aggregateValue(kind: any, path: any, targets: any) {
  const values: any = targets.map((layer: any) => getPath(layer, path)).filter((value: any) => finite(value));
  if (!values.length) return 0;
  if (kind === 'min') return Math.min(...values);
  if (kind === 'max') return Math.max(...values);
  if (kind === 'first') return values[0];
  if (kind === 'last') return values.at(-1);
  if (kind === 'sum') return values.reduce((sum: any, value: any) => sum + value, 0);
  return values.reduce((sum: any, value: any) => sum + value, 0) / values.length;
}

function evaluate(expression: any, context: any) {
  ...
  const args: any = (expression.args || []).map((arg: any) => evaluate(arg, context));
  switch (expression.op) {
    case 'add': return args.reduce((sum: any, value: any) => Number(sum) + Number(value), 0);
    case 'subtract': return Number(args[0]) - Number(args[1] || 0);
    case 'multiply': return args.reduce((product: any, value: any) => Number(product) * Number(value), 1);
...
function normalizedValue(path: any, value: any) {
...
function compile(raw: any, state: any = {}) {
  const transform: any = sanitizeTransform(raw);
  if (!transform) return { ok: false, message: 'The generated transformation is invalid', commands: [], changes: [] };
  const targets: any = orderedTargets(resolveTargets(transform.selector), transform.order, state, transform.seed);
  ...
  const byLayer: any = new Map();
  const changes: any = [];
  for (let index: any = 0; index < targets.length; index++) {
    const layer: any = targets[index];
    for (const edit of transform.edits) {
      const before: any = getPath(layer, edit.path);
      if (before === undefined) continue;
      const value: any = normalizedValue(edit.path, evaluate(edit.value, { current: before, index, layer, targets, state }));
      if (Object.is(before, value)) continue;
      const entry: any = byLayer.get(layer.id) || { layer, layerPatch: {}, contentPatch: {}, properties: [] };
...
function preview(raw: any, state: any) { return compile(raw, state); }

function apply(raw: any, state: any, meta: any = {}) {
  const result: any = compile(raw, state);
  if (!result.ok) return result;
  const applied: any = PM.Edit.apply(result.commands, {
...
function selectionSummary() {
  ...
  const unlocked: any = selected.filter((layer: any) => !layer.lock).length;
...
function keyframeSummary() {
  const count: any = selectedKeyframes().length;
  return count ? `${count} keyframe${count === 1 ? '' : 's'} selected` : 'No keyframes selected';
}

function easingResult(action: any, state: any = {}) {
  const keys: any = selectedKeyframes();
  if (!keys.length) return { ok: false, message: 'Select at least one keyframe', commands: [], changes: [] };
  const curve: any = sanitizeCurve(state[action.curveState], action.defaultCurve || DEFAULT_CURVE);
  const name: any = PM.Ease?.nameOf?.([curve[0], curve[1]], [curve[2], curve[3]]) || 'custom';
  const changes: any = keys.map((key: any) => ({
  ...
  return {
    ...
    curve, changes, commands: [{ type: 'set_easing', keyframes: keys.map((key: any) => key.i), curve }],
  };
}

function previewEasing(action: any, state: any) { return easingResult(action, state); }
function applyEasing(action: any, state: any, meta: any = {}) {
  const result: any = easingResult(action, state);
  if (!result.ok) return result;
  const applied: any = PM.Edit.apply(result.commands, {
...
function staggerTransform(mode: any = 'apply') {
  return {
    type: 'transform', mode,
    transform: {
...
const DECOMPOSE_TEXT_SCRIPT: any = `
...
function decomposeTextAction(mode: any = 'apply') {
  return {
