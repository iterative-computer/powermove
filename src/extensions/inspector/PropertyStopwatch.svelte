<script lang="ts">
  import { animateSelection } from './selection-animation';
  ...
  function toggle(event: MouseEvent): void {
    ...
    const result=animateSelection(PM,layer,[path],animated,transport.time,`${animated?'Remove animation from':'Animate'} ${label}`,value);
...
<button type="button" class="stopwatch property-stopwatch" class:on={animated} class:at-key={current}
  aria-label={`${animated ? 'Remove animation from' : 'Animate'} ${label}`} aria-pressed={animated}
  title={animated ? `Remove animation from ${label} · keep current value` : `Animate ${label}`}
