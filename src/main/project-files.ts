...
export class ProjectFiles {
  ...
  save(id: string, data: Uint8Array, selectedPath?: string): Promise<string> {
    return this.serial(async () => {
      ...
      if (!selectedPath && known && (!previous || hash(previous) !== known.hash)) {
      ...
      if (previous) await atomicWrite(destination + '1', previous);
  ...
  open(filePath: string): Promise<{ path: string; projectId: string; text: string }> {
    return this.serial(async () => {
      ...
      await this.persist();
      return { path: filePath, projectId, text };
    });
  }
}
